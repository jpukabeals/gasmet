#Code to analyze TLI GHG flux data from PAP Field station

#Script to extract Gasmet text files, regorganize them,
#and create simple figures for data interpretation.

#This script will only work for data in Madeline's OneDrive #folder following the structure that's there
#now. So it will look to specific folder names and
#file names. 
###############
mygoogledoc<-"C:/Users/jbowden/OneDrive - Madeline/OneDrive - The Land Institute/Gasmet data/2020 PAP field station gasmet data/PAP FS gasmet txt files may-november 2020"
...
#This loop extracts only the relevant data from each text file
  for(i in 1:length(fnames)){ #runs through all the files in the folder
    sdat<-read.delim(paste(fnames[i])) #opens the text files
    if(colnames(sdat)[1]=="Line"){ #The old files that were repredicted by the Gasmet folks have a strange format. This identifies those files by seeing if the first column is labeled "Line"
      ...
      keepcols<-c(2,8,11,14) #The time column and the gasses of interest are in these columns
    ...
    sdat$plot<-str_split_fixed(fnames[i], "2020|[.]",3)[,2] #create a "plot" column, and extract the plot id from the file name
    sdat$day<-str_split_fixed(fnames[i], "PAPS|2020",3)[,2] #create a "day" column, and extract the day from the file name
    sdat$code<-str_split_fixed(fnames[i], "PAPS|[.]",3)[,2] #create a "code" column, which is the plot and date combined, which can be extracted from the file name
    sdat$time<-c((1:nrow(sdat)*24)/3600) #this is the "time" in fractions of an hour, since our final units will be in CO2e/m2/hour
    shortdat<-rbind(shortdat, sdat)
  }
#shortdat$block<-substr(shortdat$plot,1,1) #Add a column for the blocks
shortdat$plot<-as.factor(shortdat$plot)
...
#this next part isn't relevant for PAP since there aren't treatments
...
#shortdat$trt<-merge(shortdat,dattrt,"plot")[,10]
...
ggplot(shortdat, aes(y=CO2, x=time, color=factor(plot)))+
  ...
  xlim(0,0.25)
################
#This creates a graph of N2o
ggplot(shortdat, aes(y=N2O, x=time, color=factor(plot)))+
  facet_grid(~day)+
  geom_point()+
  geom_line()+
  xlim(0,0.25)
...
############
#I think this now makes the same graph with the truncated time?

ggplot(tshortdat, aes(y=N2O, x=time, color=factor(plot)))+
  facet_grid(~day)+
  geom_point()+
  geom_line()+
  xlim(0,0.25)+
  ggtitle("N2o Fluxes in Kernza Alfalfa Intercrop in 2020")+
  xlab("Date")+
  ylab("N2O (micro grams N2O-N/cm3)")+
  #geom_error()
  
#save the plot
ggsave("PAPFSN2Ograph.pdf", width=8, height=5, units="in", path="C:/Users/jbowden/OneDrive - Madeline/OneDrive - The Land Institute/Gasmet data/2020 PAP field station gasmet data")
...
outlierlook<-function(gas1){
  ...
  boxplot(tshortdat[,colnum]~day, data=tshortdat, range=2)
...
#################### 
#CO2 without block
co2regressions <- dlply(tshortdat, .(day), lm, formula = CO2.C ~ time)
...
n20regressions <- dlply(tshortdat, .(day), lm, formula = N2O.N ~ time)
n20coefs <- cbind(ldply(n20regressions, coef)[,c(1,2)],
                  ldply(n20regressions, function(x){coef(summary(x))[4]})[,2])

colnames(n20coefs)<-c("day", "slope", "slope_se")
n20coefs$day2<-as.Date(as.character(n20coefs$day),"%m%d")


#This is an important figure. 
ggplot(n20coefs, aes(y=slope, x=day2))+
  geom_point()+
  geom_line()+
  ggtitle("N2O Fluxes in Kernza Alfalfa Intercrop in 2020")+
  geom_point(color='darkgreen')+
  geom_errorbar(width=2, aes(ymin=slope-slope_se, ymax=slope+slope_se))+
  xlab("Date")+
  ylab("Flux (micro grams N2O-N/cm3)")+
  scale_x_date(date_break="2 weeks", date_labels="%b %d")+
  theme(plot.title = element_text(hjust = 0.5, ="bold"),axis.text.x=element_text(angle=60,hjust=1))
  

 
#save the plot
ggsave("PAPFSN2Ofluxgraph.pdf", width=8, height=5, units="in", path="C:/Users/jbowden/OneDrive - Madeline/OneDrive - The Land Institute/Gasmet data/2020 PAP field station gasmet data")
